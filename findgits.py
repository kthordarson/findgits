...
async def main() -> None:
    ...
    try:
        if args.scanpath:
            ...
            print(f'Git Lists: {len(unified_data)} git_list_count: {len(unified_data)} Starred Repos: {len(starred_repos)} urls: {len(urls)} foundrepos: {len(foundrepos)} notfoundrepos: {len(notfoundrepos)}')
