...
async def link_existing_repos_to_stars(
...
async def populate_git_lists(
    ...
    for entry in list_data:
        ...
        try:
            ...
            repo_count = 0
            logger.warning(
                f"Could not parse repo_count '{entry.get('repo_count')}' for list {list_name}: {e}"
            )
        ...
        set_cache_entry(
            session, list_cache_key, list_cache_type, json.dumps(entry_to_cache)
        )
